...
def generatepoints(length,spacing,height=3):
    ...
    for i in range(1,int(length)):
        a=points[i-1][0]
        b=points[i-1][1]
        x=points[i-1][0]+np.array(directions[(i-1)%4][0])*(i*spacing)
        y=points[i-1][1]+np.array(directions[(i-1)%4][1])*(i*spacing)
...
def generatepointsreverse(length,spacing,height=3):
    ...
    for i in (range(1,int(length))):
        a=points[i-1][0]
        b=points[i-1][1]
        x=points[i-1][0]+np.array(directions[(i-1)%4][0])*((length-i)*spacing)
        y=points[i-1][1]+np.array(directions[(i-1)%4][1])*((length-i)*spacing)
        xnew=a+b-y
        ynew=x+b-a
        distance_travelled += np.linalg.norm(np.array([a, b]) - np.array([x, y]))
        xs.append(xnew)
...
amp_slider = Slider(amp_slider_ax, 'Search space dimension', 1, 800, valinit=amp_0,valstep=1,valfmt= "%d")
